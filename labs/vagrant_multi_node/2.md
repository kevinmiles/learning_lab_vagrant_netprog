...
1. Start the environment with `vagrant up`.  
    1. ***Note: Due to the nature of the Nexus 9000v shell, vagrant will report an error after starting each VM.  This is expected, and does not indicate a problem.  Simple enter `vagrant up` again to complete the setup successfully.  For this exercise you'll need to enter `vagrant up` a total of three times.***
